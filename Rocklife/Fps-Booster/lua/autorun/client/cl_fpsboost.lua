...
surface.CreateFont( "IceFont", {
	...
	local function fpsboost(ply)
		...
		FpsText:SetText( "Would you like to Enable FPS Boost?" )
		...
		EnableButton:SetText( "Enable It" )
		...
		EnableButton.DoClick = function() 
			...
			local ply = LocalPlayer()
				...
				chat.AddText( Color( 0, 159, 255 ), "[FpsBoost]", Color( 255, 255, 255 ), " Your FPS is now "..math.Round( 1/RealFrameTime() ).." FPS!" )
				chat.AddText( Color( 0, 159, 255 ), "[FpsBoost]", Color( 255, 255, 255 ), " If you experience worse FPS or crashing use /fpsreset to set your settings back :D Use cl_showfps 1 in console to check your FPS as well." )
				FpsMenu:Remove()
		end
		...
		chat.AddText( Color( 0, 159, 255 ), "[FpsBoost]", Color( 255, 255, 255 ), ", If you change your mind simply type /fpsboost in chat." )
			surface.PlaySound("buttons/button15.wav") 
...
hook.Add( "OnPlayerChat", "FpsBoost", function( ply, strText)
	...
	if ( strText == "/fpsboost" ) then
		if(fpsconvar >= 1) then 
			chat.AddText( Color( 0, 159, 255 ), "[FpsBoost]", Color( 255, 255, 255 ), " You've already enabled FPS Boost :D" )
		else
			fpsboost(ply)
		return true 
		end
	...
	if ( strText == "/fpsreset" ) then
		if(fpsconvar >= 1) then 
			chat.AddText( Color( 0, 159, 255 ), "[FpsBoost]", Color( 255, 255, 255 ), " You've reset your FPS Boost Settings back to Garry's Mod default values!" )
